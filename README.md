...
jobs:
  claude-response:
    ...
    steps:
      - uses: hfujikawa77/claude-code-action@beta  # Fork with OAuth support
        with:
...
steps:
  - uses: hfujikawa77/claude-code-action@beta
    with:
      direct_prompt: |
        Update the API documentation in README.md to reflect
        the changes made to the API endpoints in this PR.
...
This action is built on top of [`hfujikawa77/claude-code-base-action`](https://github.com/hfujikawa77/claude-code-base-action).
